...
public class PrepIntake extends Command{
    ...
    public void execute(){
        if(!isBack){
            ...
            if(Elevator.getInstance().getState() != ElevatorStates.INTAKE){
                CommandScheduler.getInstance()
            .schedule(new SetTargetReefLevel(ElevatorStates.INTAKE));
            } else{
                if(Wrist.getInstance().getState()!= WristStates.INTAKE && Elevator.getInstance().getPosition()>0.2 ){//TODO set actcul cross bar postion and >/<
                    Wrist.getInstance().setState(WristStates.INTAKE, ClosedLoopSlot.kSlot0);//TODO change K slot to correct one
                }
            }
    ...
    public boolean isFinished(){
        if(Wrist.getInstance().isAtState(WristStates.INTAKE,0.01) && Elevator.getInstance().isAtState(ElevatorStates.INTAKE,0.01)){//TODO change tolerance x2 (wrist + elevator)
            return true;
        }
        return false;
    }
