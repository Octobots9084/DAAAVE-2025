...
public class ScoreCoral extends Command {
    ...
    public void execute() {
        ...
        if (elevatorInPos && wristInPosition && isAligned && (coralRollers.getState() != CoralRollersState.REJECTING))
    ...
    public boolean isFinished() {
        if(coralRollers.io.hasCoral() == false)
            return true;
        return false;
    }
    ...
    public void end(boolean interrupted)
    {
        ...
        if (!interrupted) //when coral has been scored send elevator back down after 0.1 seconds
            CommandScheduler.getInstance().schedule(new WaitCommand(0.1).andThen(new SetElevatorState(ElevatorStates.INTAKE)));
    }
