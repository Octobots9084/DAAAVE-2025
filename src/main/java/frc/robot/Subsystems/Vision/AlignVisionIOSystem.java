...
public class AlignVisionIOSystem implements AlignVisionIO {
  private static AlignVisionIOSystem INSTANCE;
  ...
  public ChassisSpeeds getAlignChassisSpeeds(AlignState state) {
    ...
    try {
      if (refPosition.getX() != Transform2d.kZero.getX()
        ...
        Logger.recordOutput("Align RefPoseX", refPosition.getX());
        Logger.recordOutput("Align RefPoseY", refPosition.getY());
    ...
    Logger.recordOutput("Final Align X Speed", xSpeed);
    Logger.recordOutput("Final Align Y Speed", ySpeed);
    Logger.recordOutput("Final Align Turn Speed", turnSpeed);
    Logger.recordOutput("Final Align Turn Angle", targetDistance);
  ...
  public static void setReefOrientation(ReefTargetOrientation orientation) {
    Logger.recordOutput("Align Orientation", orientation.name());
    selectedReefOrientation = orientation;
  }

  public static void setPoleSide(ReefTargetSide side) {
    Logger.recordOutput("Align Side", side.name());
